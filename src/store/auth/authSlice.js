...
const pepe = {
  ...
  email: "pepe@mail.com",
  image:
    "https://upload.wikimedia.org/wikipedia/commons/c/c0/Jake_LaTurner_Congressional_ID_photo_%28117th%29.jpg",
  cart: [],
};
...
export const authSlice = createSlice({
  ...
  reducers: {
    ...
    onAddToCart: (state, { payload }) => {
      const found = state.user.cart.find(
        (product) => product.id === payload.id
      );
      if (found)
        state.user.cart = state.user.cart.map((product) => {
